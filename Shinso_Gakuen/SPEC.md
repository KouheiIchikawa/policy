...
- OGP/Twitter画像は `assets/ShisoGakuenKeyVisual_ja.png` を使用する。
...
- アプリページは `SoftwareApplication` として設定し、`applicationCategory` は `GameApplication`、`operatingSystem` は `iOS, Android`、公開前状態は `PreOrder` とする。
- プライバシーポリシーページは `WebPage` として設定する。
- `llms.txt` を設置し、サイト概要、主要ページ、作品概要、アプリ公開予定、コミック説明を記載する。
- `sitemap.xml` を設置し、主要6ページを登録する。
...
- `Shinso_Gakuen/app/assets/favicon/`
...
- iOS: `2026年6月中リリース予定`
- Android: `2026年6月中リリース予定`

`COMING SOON` はテキスト表示ではなく、サイトイメージに合わせた横長アイコン画像として表示する。

使用画像:

- `Shinso_Gakuen/assets/coming-soon-icon.png`
- `Shinso_Gakuen/app/assets/coming-soon-icon.png`

## キービジュアル仕様

トップのキービジュアルには、ふわっと明滅するフェードアニメーションを設定する。

- 画像ファイル `ShisoGakuenKeyVisual_ja.png` と `ShisoGakuenKeyVisual_en.png` は勝手に差し戻さない。
